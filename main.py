...
def parse_args():
    ...
    parser.add_argument('--alpha',default=0.35 ,type=float, help='alpha PS') # 0.8
    parser.add_argument('--beta',default=0.40 ,type=float, help='beta TF')  # 0.95
    parser.add_argument('--gamma',default=0.25 ,type=float, help='gamma CE')  # 0.95
    parser.add_argument('--temp',default=6.0 ,type=float, help='Temperature TF')
...
def main_worker(gpu, ngpus_per_node, model_dir, log_dir, args):
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            net_pre = torch.nn.parallel.DistributedDataParallel(net_pre)
    elif args.gpu is not None:
        torch.cuda.set_device(args.gpu)
        net = net.cuda(args.gpu)
        net_pre = net_pre.cuda(args.gpu)
    else:
        # DataParallel will divide and allocate batch_size to all available GPUs
        net = torch.nn.DataParallel(net).cuda()
        net_pre = torch.nn.DataParallel(net_pre).cuda()
    ...
    if args.PSKD:
        ...
        criterion_KD = nn.KLDivLoss().cuda(args.gpu)
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        if args.PSKD:
            ...
            #  Alpha_t update
            #---------------------------------------------------
            alpha = args.alpha * ((epoch + 1) / args.end_epoch)
            alpha = max(0, alpha)
        else:
            alpha = -1
        #---------------------------------------------------
    ...
    for batch_idx, (inputs, targets, input_indices) in enumerate(train_loader):
        ...
        if args.PSKD:
            ...
            if False:
                ...
                soft_targets = ((1 - alpha) * targets_one_hot) + (alpha * all_predictions[input_indices])
                ...
                loss_pskd = criterion_CE_pskd(outputs, soft_targets)

            old_targets = all_predictions[input_indices]
            old_targets = old_targets.cuda()

            outputs = net(inputs)
            softmax_output = F.softmax(outputs, dim=1)

            # loss_ce = (1. - alpha - args.beta) * criterion_CE(outputs, targets)
            # loss_ce = 0.10 * criterion_CE(outputs, targets)
            loss_ce = args.gamma * criterion_CE(outputs, targets)

            # loss_pskd = alpha * (criterion_KL(torch.log_softmax(outputs, dim=1), torch.softmax(old_targets, dim=1)))
            # loss_pskd = 0.45 * (criterion_KL(torch.log_softmax(outputs, dim=1), torch.softmax(old_targets, dim=1)) / batch_size)
            # loss_pskd = 0.45 * (criterion_KL(torch.log_softmax(outputs, dim=1), torch.softmax(old_targets, dim=1).detach()))
            loss_pskd = args.alpha * (criterion_KL(torch.log_softmax(outputs, dim=1), torch.softmax(old_targets, dim=1).detach()))

            ####
            with torch.no_grad():
                outputs_pre = net_pre(inputs)

            # loss_tfkd = args.beta * (criterion_KL(torch.log_softmax(outputs/args.temp, dim=1), torch.softmax(outputs_pre/args.temp, dim=1)) * (args.temp**2))
            # loss_tfkd = 0.45 * (criterion_KL(torch.log_softmax(outputs/args.temp, dim=1), torch.softmax(outputs_pre/args.temp, dim=1)) * (args.temp**2) / batch_size)
            # loss_tfkd = 0.45 * (criterion_KL(torch.log_softmax(outputs/args.temp, dim=1), torch.softmax(outputs_pre/args.temp, dim=1).detach()) * (args.temp**2))
            loss_tfkd = args.beta * (criterion_KL(torch.log_softmax(outputs/args.temp, dim=1), torch.softmax(outputs_pre/args.temp, dim=1).detach()) * (args.temp**2))
            ...
            loss = loss_ce + loss_pskd + loss_tfkd
        else:
            outputs = net(inputs)
            loss = criterion_CE(outputs, targets)

        train_losses.update(loss.item(), inputs.size(0))
        ce_losses.update(loss_ce.item(), inputs.size(0))
        ps_losses.update(loss_pskd.item(), inputs.size(0))
        tf_losses.update(loss_tfkd.item(), inputs.size(0))
        ...
        progress_bar(epoch,batch_idx, len(train_loader), args, 'lr: {:.1e} | alpha: {:.3f} | loss: {:.3f}/{:.3f}/{:.3f}/{:.3f} | top1_acc: {:.3f} | top5_acc: {:.3f} | correct/total({}/{})'.format(
            current_LR, alpha, train_losses.avg, ce_losses.avg, ps_losses.avg, tf_losses.avg, train_top1.avg, train_top5.avg, correct, total))

    if args.distributed:
        dist.barrier()
    
    logger = logging.getLogger('train')
    logger.info('[Rank {}] [Epoch {}] [PSKD {}] [lr {:.1e}] [alpha {:.3f}] [train_loss {:.3f}] [train_top1_acc {:.3f}] [train_top5_acc {:.3f}] [correct/total {}/{}]'.format(
        args.rank,
        epoch,
        args.PSKD,
        current_LR,
