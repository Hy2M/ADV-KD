...
#----------------------------------------------------
from models.network import get_network, get_discriminator
...
def parse_args():
    ...
    parser.add_argument('--lr_decay_schedule', default=[100, 150], nargs='*', type=int, help='when to drop lr')
    parser.add_argument('--weight_decay', default=5e-4, type=float, help='weight_decay')
    parser.add_argument('--start_epoch', default=0, type=int, help='manual epoch number')
    # parser.add_argument('--end_epoch', default=300, type=int, help='number of training epoch to run')
    parser.add_argument('--end_epoch', default=200, type=int, help='number of training epoch to run')
    ...
    parser.add_argument('--alpha',default=0.8 ,type=float, help='alpha PS') # 0.8 #.35
...
def main_worker(gpu, ngpus_per_node, model_dir, log_dir, args):
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            discriminator_net.cuda(args.gpu)
            ...
            discriminator_net = torch.nn.parallel.DistributedDataParallel(discriminator_net)
        ...
        discriminator_net = discriminator_net.cuda(args.gpu)

    else:
        # DataParallel will divide and allocate batch_size to all available GPUs
        net = torch.nn.DataParallel(net).cuda()
        net_pre = torch.nn.DataParallel(net_pre).cuda()
        discriminator_net = torch.nn.DataParallel(discriminator_net).cuda()
    ...
    if args.PSKD:
        ...
        criterion_KD = nn.KLDivLoss(reduction="batchmean").cuda(args.gpu)
    ...
    for batch_idx, (inputs, targets, input_indices) in enumerate(train_loader):
        ...
        if args.PSKD:
            ...
            if batch_idx % 5 == 0:
                ...
                loss = loss_ce + loss_pskd + loss_tfkd + adv_loss
                ad_losses.update(adv_loss.item(), inputs.size(0))
            else:
